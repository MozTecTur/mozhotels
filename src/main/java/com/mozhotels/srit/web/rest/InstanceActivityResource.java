...
import com.mozhotels.srit.repository.InstanceActivityRepository;
import com.mozhotels.srit.repository.search.InstanceActivitySearchRepository;
...
public class InstanceActivityResource {
    ...
    @RequestMapping(value = "/instance-activities",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<InstanceActivity>> getAllInstanceActivities(Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to get a page of InstanceActivities");
        Page<InstanceActivity> page = instanceActivityRepository.findAll(pageable); 
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/instance-activities");
    ...
    @RequestMapping(value = "/_search/instance-activities",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<InstanceActivity>> searchInstanceActivities(@RequestParam String query, Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to search for a page of InstanceActivities for query {}", query);
        Page<InstanceActivity> page = instanceActivitySearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/instance-activities");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
